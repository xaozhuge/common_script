...
#备份数据表 --no-create-info 表示取消创建表sql，默认存在 --where where条件限制
/usr/local/mysql/bin/mysqldump -h$ip2 -u$username2 -P$port2 -p$password2 --no-create-info  $databases $tables --where="id >= 10 and id < 20 " >> $tableSqlPath 2>&1 | removewarning
